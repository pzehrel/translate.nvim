...
test("custom llm client receives translation options", function()
  ...
  require("translate.llm").translate("hello", config, function(err, translated)
    assert(err == nil)
    result = translated
  end)
...
test("llm client reports missing endpoint and model", function()
  ...
  require("translate.llm").translate("hello", config, function(err)
    message = err
  end)
...
test("api key is read from configured environment variable", function()
  local previous = vim.env.TRANSLATE_NVIM_TEST_KEY
  vim.env.TRANSLATE_NVIM_TEST_KEY = "secret-from-env"

  local value = require("translate.llm").resolve_api_key({
    api_key = nil,
    api_key_env = "TRANSLATE_NVIM_TEST_KEY",
  })

  vim.env.TRANSLATE_NVIM_TEST_KEY = previous
